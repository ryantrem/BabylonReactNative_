...
namespace Babylon
{
    ...
    public:
        ...
        Native(JSGlobalContextRef jsContext, ANativeWindow* windowPtr)
        {
            ...
            JsRuntime::Initialize(*m_env, dispatchFunction);

            dispatchFunction([windowPtr](Napi::Env env) {
        ...
        void Refresh(ANativeWindow* windowPtr)
        {
            auto width = static_cast<size_t>(ANativeWindow_getWidth(windowPtr));
            auto height = static_cast<size_t>(ANativeWindow_getHeight(windowPtr));
            auto& runtime = JsRuntime::GetFromJavaScript(*m_env);
    ...
    private:
        using looper_scheduler_t = arcana::looper_scheduler<sizeof(std::shared_ptr<Napi::Env>) + sizeof(std::function<void(Napi::Env)>)>;
